...
class NPYDepressionDataset(Dataset):
    ...
    def __init__(self, features_base_path, split='train'):
        ...
        # 加载抑郁和非抑郁特征文件
        depressed_path = os.path.join(self.split_path, 'depressed.npy')
        non_depressed_path = os.path.join(self.split_path, 'non_depressed.npy')
...
class DepressionTrainer:
    ...
    def validate_epoch(self):
        ...
        # 计算其他指标
        precision = precision_score(all_labels, all_predictions, average='binary', zero_division=0) if len(all_labels) > 0 else 0
        recall = recall_score(all_labels, all_predictions, average='binary', zero_division=0) if len(all_labels) > 0 else 0
...
def debug_dataset_shapes(features_base_path):
    ...
    for split in ['train', 'val']:
        ...
        if os.path.exists(split_path):
            depressed_path = os.path.join(split_path, 'depressed.npy')
            non_depressed_path = os.path.join(split_path, 'non_depressed.npy')
            ...
            if os.path.exists(non_depressed_path):
                non_depressed_features = np.load(non_depressed_path)
                print(f"{split}/non_depressed.npy - 形状: {non_depressed_features.shape}, 数据类型: {non_depressed_features.dtype}")
